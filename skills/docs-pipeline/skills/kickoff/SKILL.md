...
- Recon chạy **TRƯỚC** `interview-me` và **TRƯỚC** freeze gate — **KHÔNG đụng hash/freeze/lock** (ngoài việc `project_type` đã set).

## Elicitation (🟢 nhẹ)

- **Brownfield:** recon (mục trên) chạy **TRƯỚC** — bucket ground câu hỏi (interview-me hỏi dựa trên code thật, không moi từ đầu). **Greenfield:** bỏ qua recon, y flow cũ.
- `interview-me` _nhẹ_ — thu brief + non-negotiable. Một câu hỏi một lúc, có guess.
