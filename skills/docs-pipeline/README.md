...
```
docs-pipeline/                      # skills-directory plugin — auto-load qua .claude-plugin/plugin.json
├── .claude-plugin/
│   └── plugin.json                 # name: docs-pipeline → skill gọi dạng /docs-pipeline:<name>
...
Skill được discover dạng **plugin** (`/docs-pipeline:kickoff` …). Mỗi SKILL.md tham chiếu shared refs qua `../../references/<name>.md` — single-source, không copy-paste protocol.
