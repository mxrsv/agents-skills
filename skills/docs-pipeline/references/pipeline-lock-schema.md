...
flags:
  pending_arch_sync: false      # /adr đổi-kiến-trúc set khi ARCHITECTURE ĐÃ frozen (§ freeze/adr).
...
6. **`grill-with-docs` CẤM chạm file này** — grill chạy **inline chỉ đề-xuất text** (xem `elicitation-contract`); chỉ phase/update skill ghi lock theo schema. Backstop: grill lỡ ghi frozen doc/ADR → gate hash + manifest cross-check phát hiện (`freeze-protocol`).
