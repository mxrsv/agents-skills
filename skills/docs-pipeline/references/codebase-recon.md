...
> **Nguyên tắc:** brownfield có sẵn kiến trúc/luật NẰM TRONG CODE, nhưng pipeline greenfield không đọc code → docs mù. Recon lấp đúng khoảng đó: **mô tả cái đang có** để ground elicitation + chặn docs mâu thuẫn thực tại. Recon = **descriptive scaffolding trong CONTEXT (living)**, KHÔNG phải contract, KHÔNG frozen doc mới.

## Khi nào chạy

- **Chỉ brownfield** (`project_type: brownfield`, sau detection + người confirm ở `/kickoff`).
- **TRƯỚC `interview-me`** — bucket recon ground câu hỏi (hỏi "thấy anh enforce X ở `auth/guard.ts:42`, có phải hard-constraint?" thay vì moi từ đầu).
- **TRƯỚC freeze gate PRINCIPLES** — recon **KHÔNG đụng hash / freeze / lock** (chỉ đọc + trả text).
...
- **KHÔNG mâu thuẫn với grill-inline.** `grill-with-docs` bỏ subagent vì subagent đó là **write-sandbox** (đã thành đảm-bảo-giả, hạ về inline). Recon-subagent đứng trên **trục KHÁC — read-isolation, không phải write-suppression** — và vẫn trả **text-only** cho **parent là bên ghi duy nhất**. Hai quyết định không chọi nhau.
...
- Distill vào frozen doc theo phase: bucket A → PRINCIPLES (**chỉ hard-constraint**, giữ nhẹ) · B → BUSINESS-FLOW (`/product`) · C → ARCHITECTURE (`/architecture`). **Không frozen doc mới** — các doc frozen sẵn hấp thụ current-state.
...
3. Parent ghi text vào `CONTEXT.md` (section có nhãn). Recon KHÔNG đụng hash/freeze/lock ngoài `project_type`.
4. Phase sau distill bucket qua CONTEXT như thường; không ai đẻ frozen doc recon riêng.
