# adk (ADR-driven kit) — docs-bootstrap pipeline + hệ elicitation (thay BMad)

> Tên cũ: `docs-pipeline` (đổi tên 2026-07-10). Nay gom cả 4 skill vệ tinh elicitation (`interview`, `brainstorm`, `grill`, `grill-docs`) vào cùng package — một nhận diện `adk:*` duy nhất.
...
/kickoff → /product → /architecture → /requirements → REQUIREMENTS.md (frozen contract)
   ⇧ elicitation: adk:interview · adk:grill-docs        (adk:brainstorm đứng ngoài pipeline, KHÔNG trong elicitation-contract)
...
└── skills/
    ...
    ├── interview/     SKILL.md      # vệ tinh — làm rõ intent trước mọi thứ (fork từ agent-skills:interview-me)
    ├── brainstorm/    SKILL.md      # vệ tinh — idea → design (fork từ superpowers:brainstorming 6.1.1)
    ├── grill/         SKILL.md      # vệ tinh — chất vấn plan/design (trước: skill `grilling`)
    └── grill-docs/    SKILL.md      # vệ tinh — grill trả text; parent ghi CONTEXT, ADR đi qua /adk:adr
```
...
Back-compat: `~/.agents/skills/grilling` + `grill-with-docs` giờ là symlink trỏ NGƯỢC vào package này (nội dung thật đã dời vào đây).
...
## Engine elicitation (TRONG package từ 2026-07-10)

`adk:interview` + `adk:grill-docs` là engine elicitation của pipeline, nay nằm **trong** package (`elicitation-contract`). `adk:brainstorm` là vệ tinh dùng ngoài pipeline — trong pipeline chỉ giữ "2-3 approaches" inline.
