...
This skill closes the gap before it costs anything. It runs before the caller distills a spec or decision: ask one question at a time, attach your best guess, and continue until you can predict what the user is going to say before they say it.
...
Return the confirmed intent to the phase skill or caller that invoked `adk:interview`. Do not persist it or invoke a downstream skill on the caller's behalf.
...
- [ ] The confirmed intent was returned to the caller without being persisted or routed downstream
