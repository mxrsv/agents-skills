...
- Write user-facing artifacts (plans, specs, documentation, comments, PR material) in **English** — except specs written by `brainstorm`, plans written by `planning`, and `/explain` output, which are always Vietnamese.
- Be concise and direct. Lead with the result, no preamble, no closing recap. Get straight to the point, no fluff.
