...
1. **Always answer in Vietnamese** — the prompt's language never changes the output language. This holds for normal replies and for user-facing artifacts (documentation, PR material); task plans under `docs/plans/` stay in English (D4). Only the `/explain` skill or an explicit request for another language overrides this. Write Vietnamese with full diacritics; never strip them to ASCII ("không", never "khong").
